			...
			>
				...
				<div class="flex items-center justify-between mb-1">
					<div class="flex items-center gap-2 font-medium text-gray-700 dark:text-gray-300">
						{#if section.id === 'web_search'}
							<span>🔍</span>
						{:else if section.id === 'career_research'}
							<span>🎯</span>
						{:else if section.id === 'kb_search'}
							<span>📚</span>
						{:else}
							<span>⚡</span>
						{/if}
				...
				{#if section.thinking}
					<div class="mt-2">
						<button
							...
							on:click={() => toggleThinking(section.id)}
						>
							<span class="transform transition-transform {expandedThinking[section.id] ? 'rotate-90' : ''}"
								>▶</span
							>
							Thinking
						</button>
						{#if expandedThinking[section.id]}
							<pre
								class="mt-1 text-xs text-gray-500 dark:text-gray-400 whitespace-pre-wrap break-words max-h-40 overflow-y-auto">{section.thinking}</pre>
						{/if}
					</div>
				{/if}
				...
				{#if section.result}
					<div class="mt-2">
						<button
							class="text-xs text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 flex items-center gap-1"
							on:click={() => toggleResult(section.id)}
						>
							<span class="transform transition-transform {expandedResult[section.id] ? 'rotate-90' : ''}"
								>▶</span
							>
							Agent result
						</button>
						{#if expandedResult[section.id]}
							<pre
								class="mt-1 text-xs text-gray-600 dark:text-gray-300 whitespace-pre-wrap break-words max-h-60 overflow-y-auto">{section.result}</pre>
						{/if}
					</div>
				{/if}
