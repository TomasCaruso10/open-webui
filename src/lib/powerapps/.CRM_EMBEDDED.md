...
1.  **Store persistente (`src/lib/powerapps/stores/crmContext.ts`)**:
    ...
    **Persistencia via sessionStorage** (bugfix): El store se hidrata desde `sessionStorage` al cargar el módulo y sincroniza cada cambio automáticamente. Esto resuelve el problema de que al recargar el iframe (F5), los URL params (`folder_id`, `folder_name`) ya no están disponibles porque SvelteKit los reemplazó durante la navegación interna, y el store en memoria se reseteaba a `null`, causando que se mostrara la sidebar normal en lugar de la CrmSidebar.

    `sessionStorage` fue elegido sobre `localStorage` porque es **per-tab**: cada pestaña/iframe tiene su propio storage, permitiendo que dos contactos abiertos en pestañas paralelas mantengan contextos independientes. Además, se limpia automáticamente al cerrar la pestaña, evitando datos huérfanos.
    ...
    La API pública del store (`crmContext.set()`, `$crmContext`, `isCrmEmbedded`) no cambia. Todo el código consumer sigue funcionando sin modificaciones.
...
2. CrmContextService crea/encuentra carpeta → setea selectedFolder + crmContext (se persiste en sessionStorage)
...
7. Iframe reload (F5) → crmContext se hidrata desde sessionStorage → CrmSidebar se muestra correctamente
```
...
## Feature 6: Sección Knowledge en CrmSidebar

**Descripción:**
La CrmSidebar (Feature 4) mostraba únicamente los chats de la carpeta del contacto. Aunque la KB del contacto se inyecta automáticamente en los chats via el middleware (Feature 5), el orientador no tenía visibilidad de qué archivos existen en la KB ni acceso directo para subir documentos. Se agrega una sección colapsable "Knowledge" en la sidebar que muestra las KBs vinculadas a la folder y sus archivos.
...
**Sin cambios en:** Backend de Open WebUI, `crmContext.ts`, `CrmContextService.ts`, ni ningún componente existente de Open WebUI.

**Componentes reutilizados (sin modificaciones):**
- `Collapsible` (`$lib/components/common/Collapsible.svelte`) — toggle colapsable con animación slide
- `Database` icon — ícono para KB
- `DocumentPage` icon — ícono para archivos
- `ChevronDown` / `ChevronRight` icons — expand/collapse de archivos por KB
- `Spinner`, `Loader`, `Tooltip` — ya importados previamente en CrmSidebar

**APIs reutilizadas (sin modificaciones):**
- `getFolderById` (`$lib/apis/folders`) — ya importada, obtiene `folder.data.files` con las KB references
- `searchKnowledgeFilesById` (`$lib/apis/knowledge`) — lista paginada de archivos de una KB

**Detalle Técnico:**

1.  **Ubicación en el layout**: La sección Knowledge se inserta entre el botón "New Chat" y la lista de chats, dentro del área scrollable. Cuando está colapsada ocupa una sola línea. La lista de chats mantiene `flex-1` para llenar el espacio restante.

2.  **Carga de datos (lightweight)**:
    `initKnowledgeCollections()` se ejecuta junto con `initFolderChatList()` cuando la sidebar se abre. Lee las KB references directamente de `folder.data.files` (la misma llamada a `getFolderById`), filtrando por `type === 'collection'`. No hace llamadas adicionales a APIs de knowledge hasta que el usuario expande una KB.
...
3.  **Expansión lazy de archivos**:
    Los archivos de una KB solo se cargan cuando el usuario hace click en el chevron de expansión. Se usa `searchKnowledgeFilesById` con paginación (infinite scroll via `Loader`). Solo una KB puede estar expandida a la vez.

4.  **Navegación a detalle de KB**:
    Click en el nombre de la KB navega a `/workspace/knowledge/{id}`, la página nativa de Open WebUI donde el usuario puede subir archivos, editar contenido, y gestionar la KB completa.

5.  **Visibilidad condicional**:
    La sección se oculta completamente si no hay KBs vinculadas a la folder (`{#if knowledgeCollections.length > 0 || knowledgeLoading}`). No ocupa espacio visual si la folder no tiene KB asociada.

**Layout visual:**

```
┌───────────────────────────────┐
│ [logo] Contact Name    [<<]  │  ← Header (sin cambios)
├───────────────────────────────┤
│ [+] New Chat                  │  ← Botón New Chat (sin cambios)
├───────────────────────────────┤
│ ▼ Knowledge (1)               │  ← NUEVO: sección colapsable
│   [DB] Docs: Juan Pérez  [>] │  ← Click nombre → /workspace/knowledge/{id}
│     [doc] informe.pdf         │  ← Archivos expandidos (lazy-loaded)
│     [doc] cv.docx             │
├───────────────────────────────┤
│ Today                         │  ← Lista de chats (sin cambios)
│   Chat about employment...    │
│   Follow-up on documents...   │
├───────────────────────────────┤
│ [avatar] User Name            │  ← User menu (sin cambios)
└───────────────────────────────┘
```
...
| Decisión | Elección | Justificación |
|----------|----------|---------------|
| ¿Dónde ubicar la sección? | Entre New Chat y chat list | Visible al abrir la sidebar, no requiere scroll. Colapsable para no robar espacio a los chats. |
| ¿Cómo obtener las KBs? | De `folder.data.files` (ya resuelto por getFolderById) | Zero llamadas extra al backend. Reutiliza el mecanismo nativo de linkeo folder↔KB. |
| ¿Cuándo cargar archivos? | Lazy: solo al expandir una KB | Minimiza llamadas al backend. El orientador ve la lista de archivos solo cuando la necesita. |
| ¿Crear componente nuevo? | No, inline en CrmSidebar | Coherente con la decisión de Feature 4: mantener todo en un solo archivo para minimizar footprint del fork. |
| ¿Cómo navegar a la KB? | `goto('/workspace/knowledge/{id}')` | Reutiliza la UI existente completa (upload, edit, delete, etc.) sin desarrollar nada extra. |

**Flujo completo:**
```
1. Orientador abre chat desde CRM → sidebar se muestra
2. showSidebar subscriber ejecuta initKnowledgeCollections()
3. Se lee folder.data.files → se filtran las type="collection" → knowledgeCollections = [{...}]
4. Sección "Knowledge (1)" aparece colapsable, abierta por defecto
5. Orientador ve "Docs: Juan Pérez" con ícono DB
6. Click en nombre → navega a /workspace/knowledge/{id} → puede subir archivos
7. Click en chevron [>] → se expande → llama searchKnowledgeFilesById → muestra archivos
8. Click en archivo → navega a la misma página de KB (detalle)
9. Si no hay KBs vinculadas → sección no se muestra
```
